...
type Chat = {
  ...
  vendorName?: string;
  adminName?: string;
  conversation_type?: string;   // 🔥 ADD THIS
  second_admin_id?: number;     // 🔥 ADD THIS
  user_id?: number;
  vendor_id?: number;
  admin_id?: number;
  lastMessage?: string;
...
// ✅ vendor-admin
if (chat.conversation_type === "vendor-admin") {
  targetId = chat.vendor_id ?? 0;
  targetName = chat.vendorName || "Vendor";
}

// ✅ admin-admin (FIXED PROPERLY)
else if (chat.conversation_type === "admin-admin") {
  if (chat.admin_id === currentUserId) {
    targetId = chat.second_admin_id ?? 0;
  } else {
...
const createConversation = async () => {
  ...
  const openChat = (
    ...
    router.push(
  `/admin/vendor-chat/detail?conversationId=${conversationId}&userId=${userId}&name=${encodeURIComponent(
    name
  )}`
);
  };
...
.filter((chat) =>
  ...
  chat.conversation_type === "vendor-admin"
)
  .map((chat, index) => {
let name = "Unknown";
let role = "UNKNOWN";
